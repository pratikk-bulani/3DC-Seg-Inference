...
class InferenceDataset(Davis):
    def __init__(self, video_file, results_dir, cfg):
        ...
        super(InferenceDataset, self).__init__(root = self.frames_dir, mode = 'test', resize_mode = cfg.INPUT.RESIZE_MODE_TEST, resize_shape = cfg.INPUT.RESIZE_SHAPE_TEST, tw = cfg.INPUT.TW, max_temporal_gap = cfg.DATASETS.MAX_TEMPORAL_GAP, imset = cfg.DATASETS.IMSET)
    ...
    def create_sample_list(self):
        img_list = sorted(list(glob.glob(os.path.join(self.frames_dir, '*.png')))) # stores all the images of a video
